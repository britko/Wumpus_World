...
class Agent:
    def __init__(self):
        ...
        self.moves=[]   # explore_world에 대한 행동을 추가하는 리스트, 금 찾으면 되돌아돌 때 reverse해서 for문 돌려 실행한다.
    ...
    #
    def get_action(self):
        actions = ['MOVE_RIGHT', 'MOVE_LEFT', 'MOVE_UP', 'MOVE_DOWN']   # 동작열
    ...
    def explore_world(self):  #move와 tb에 따라 moves에 action을 추가하고 action을 반환
        ...
        if self.move==1:
            if self.border==False:
                ...
                self.move_p=actions[0]  #move_p를 MOVE_RIGHT로 하고
                return actions[0]   # MOVE_RIGHT을 리턴
            elif self.border==True:  # 벽에 부딪히면
                self.num = random.randint(0,2)  # 같은 방향을 제외한 3가지 방향중 한곳 설정
                self.nextnum.remove(self.move) # 1~4가 들어있는 배열에서 현재 방향을 가르키는 숫자 제거
                self.nextangle = self.nextnum[self.num]  # 다음 행동의 방향을 결정
                self.change=actions[:]  # 현재 행동을 제외하고 나머지 행동 복사
                del self.change[0]
                self.nextnum.append(self.move) # 현재 방향 제거한 방향 대입하고 다음에 다시 사용하기위해 채워넣음

                self.moves.append(self.change[self.num])   
                self.move_p=self.change[self.num]
                self.move = self.nextangle  # 다음 행동의 방향 대입
                self.f==True
                return self.change[self.num]  

        if self.move==2:
            if self.border==False:
                self.moves.append(actions[1])   # moves 리스트에 MOVE_UP 추가
                self.move_p=actions[1]  
                return actions[1]   # MOVE_LEFT을 리턴
            elif self.border==True:  # 벽에 부딪히면
                self.num = random.randint(0,2)  # 같은 방향을 제외한 3가지 방향중 한곳 설정
                self.nextnum.remove(self.move) # 1~4가 들어있는 배열에서 현재 방향을 가르키는 숫자 제거
                self.nextangle = self.nextnum[self.num]  # 다음 행동의 방향을 결정
        ...
        if self.move==3:
            if self.border==False:
                self.moves.append(actions[2])   # moves 리스트에 MOVE_UP 추가
                self.move_p=actions[2]  
                return actions[2]   # MOVE_UP을 리턴
            elif self.border==True:  # 벽에 부딪히면
                self.num = random.randint(0,2)  # 같은 방향을 제외한 3가지 방향중 한곳 설정
                self.nextnum.remove(self.move) # 1~4가 들어있는 배열에서 현재 방향을 가르키는 숫자 제거
                self.nextangle = self.nextnum[self.num]  # 다음 행동의 방향을 결정
        ...
        if self.move==4:
            if self.border==False:
                self.moves.append(actions[3])   # moves 리스트에 MOVE_UP 추가
                self.move_p=actions[3]  
                return actions[3]   # MOVE_DOWN을 리턴
            elif self.border==True:  # 벽에 부딪히면
                self.num = random.randint(0,2)  # 같은 방향을 제외한 3가지 방향중 한곳 설정
                self.nextnum.remove(self.move) # 1~4가 들어있는 배열에서 현재 방향을 가르키는 숫자 제거
                self.nextangle = self.nextnum[self.num]  # 다음 행동의 방향을 결정
    ...
    def make_move(self): 
        actions = ['MOVE_RIGHT', 'MOVE_LEFT', 'MOVE_UP', 'MOVE_DOWN']
        temp=[]
        if self.border==False and self.move==1:
                self.moves.append(actions[0])   # moves 리스트에 MOVE_UP 추가
                self.move_p=actions[0]  
                return actions[0]
        if self.border==False and self.move==2:
                self.moves.append(actions[1])   # moves 리스트에 MOVE_UP 추가
                self.move_p=actions[1]  
                return actions[1]
        if self.border==False and self.move==3:
                self.moves.append(actions[2])   # moves 리스트에 MOVE_UP 추가
                self.move_p=actions[2]  
                return actions[2]
        if self.border==False and self.move==4:
                self.moves.append(actions[3])   # moves 리스트에 MOVE_UP 추가
                self.move_p=actions[3]  
                return actions[3]
