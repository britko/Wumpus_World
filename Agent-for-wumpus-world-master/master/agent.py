...
class Agent:
    def __init__(self):
        ...
        self.arrow = 3  #화살 개수 3개
        self.arrow_fired=False  # 화살을 쏘았느냐? 처음엔 False로 초기화
        self.visited=[] #  
        self.a=False    #
    ...
    def get_action(self):
        ...
        if self.counter>999:    # 동작횟수가 1000이상이면 종료
            return "QUIT"
        if self.shoot in extras and self.arrow_fired==False:    # shoot이 화살 동작열 안에 있고 화살을 쏘지 않았으면
            if self.move_p==actions[0]: # move_p가 MOVE_UP이면 last_move는 MOVE_DOWN
                self.last_move=actions[1]
            if self.move_p==actions[1]: # move_p가 MOVE_DOWN이면 last_move는 MOVE_UP
            ...
            self.step_back=True # 뒤로 이동
            self.arrow_fired=True   # 화살 사용
            return self.shoot   # shoot을 리턴
        
        if self.step_back==True:    # step_back하면
            self.step_back=False    # 다시 false로 바꿔주고
            return self.last_move   # last_move 리턴
    ...
    def give_senses(self, location, breeze, stench):
        ...
        if stench==True:    #악취있고
            if self.arrow_fired==False: #화살 안쐈으면
